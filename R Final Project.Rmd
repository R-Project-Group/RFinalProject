...
pacman::p_load(tidyverse, dplyr, ggplot2, ggthemes, data.table, lubridate,
               ...
               plotly, ggpubr, vistime, RCurl, RJSONIO, sf, tigris, devtools)
library(dplyr)
```
...
sdl8 <- sdl7 %>%
  ...
  select(-ï..FIPS, -Estimate..HOUSING.OCCUPANCY..Total.housing.units..Homeowner.vacancy.rate, -Estimate..HOUSING.OCCUPANCY..Total.housing.units..Rental.vacancy.rate)
...
left_join(killdata_fips_yearcounty, masterCensus_s2, by="fips") %>%
...
county_map <- left_join(killdata_fips_yearcounty, counties, by = "county_fips", copy = TRUE) 
...
killdata_raceanalysis <- killdata_clean %>%
  filter(race == 'B') %>%
  group_by(mod_fips) %>%
  dplyr::summarise(totalKillings = n()) %>%
  dplyr::rename(fips = mod_fips)
>>>>>>> parent of 2a91750 (Code for Hersh)
