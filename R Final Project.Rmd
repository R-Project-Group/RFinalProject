...
count.N.per.column <- ldply(sdl2, function(c) min())
...
```{r}
set.seed(1)
X <- model.matrix(totalKillings~.,data=sdl2)

is.na(sdl2$totalKillings)
help(model.matrix)


[,-3]
Y <- sdl1$killingRate
fit.lasso.0 <- cv.glmnet(X,Y, nfolds = 10, intercept = T)
plot(fit.lasso.0)
