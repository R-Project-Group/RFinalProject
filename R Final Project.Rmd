...
fipsLookUp <- read.csv("FIPSCountyStateLookUp.csv", colClasses ='character') # From https://www.nrcs.usda.gov/wps/portal/nrcs/detail/national/home/?cid=nrcs143_013697
...
# Step 1 cleaning; only keep columns that have 'percentage' data
sdl2 <- sdl1 %>%
...
# Flag any column where the minimum is above 100
...
# Flag any column where the maximum is above 100
...
sdl9sub <- sdl8sub %>%
   mutate_all(~replace(., is.na(.), 0)) # glmnet doesn't play with NAs, so coalesce all NAs into 0
...
fit.min.lm <- lm(killingRateLog~., data=data.fl.sub) # debiased or relaxed LASSO
