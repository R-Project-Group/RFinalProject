...
killdata_raceanalysis <- killdata_clean %>%
  ...
  na.omit() %>%
  mutate(blacksRatio = as.numeric(totalBlackKillings)/as.numeric(blackPop)) %>%
...
killdata_FIPS2 <- killdata_FIPS %>%
  left_join(masterCensus_s2, by="fips") %>%
  na.omit() %>%
  mutate(overallRatio = as.numeric(totalKillings)/as.numeric(Estimate..SEX.AND.AGE..Total.population)) %>%
...
plot(x = killdata_raceanalysis$blacksRatio, 
     ...
     cex  = 0.8,    # size
     col  = "blue", # color 
     xlab = "Black Ratio",  # x-axis
     ylab = "Overall Ratio",  # y-axis 
